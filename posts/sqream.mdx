---
title: How to use GPU based Scream DB
tags:
  - database
  - sqream
  - python
...
[ScreamDB](https://www.sqream.com/) is a database system that uses a GPU (graphics processing unit) to accelerate database operations. It is designed to be used with Python code and can be integrated into a Python application using the Python bindings provided by the ScreamDB project.
...
A problem in which a specific worker does not operate (a problem in which the daemon dies) occurred while operating the SQreamDB server.
What is the pointers to be checked and solutions when the problem occurs ?
...
If none of these steps help to resolve the issue, you may need to seek further assistance from SQreamDB support or a qualified system administrator.
